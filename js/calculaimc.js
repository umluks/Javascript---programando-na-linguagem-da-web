function calculaImc() {
  ...
  for (let i = 0; i < varPacientes.length; i++) {
    ...
    let peso = parseFloat(varPeso.textContent); // Obtém o conteúdo de texto do elemento peso e converte para número

    let varAltura = paciente.querySelector(".info-altura"); // Seleciona o elemento que contém a altura do paciente
    let altura = parseFloat(varAltura.textContent); // Obtém o conteúdo de texto do elemento altura e converte para número
    ...
    if (pesoValido && alturaValida) {
      ...
      varImc.textContent = imc.toFixed(2); // Define o texto do elemento IMC com o valor calculado
    } else {
